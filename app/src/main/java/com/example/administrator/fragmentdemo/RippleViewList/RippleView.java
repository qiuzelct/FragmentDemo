package com.example.administrator.fragmentdemo.RippleViewList;
...
public class RippleView extends Button {
    ...
    private float mAlphaFactor; //透明度
    private float mDensity; //密度
    private float mRadius;  // 半径
    private float mMaxRadius; //最大半径

    private int mRippleColor; //颜色
    private boolean mIsAnimating = false;
    private boolean mHover = true;



    private RadialGradient mRadialGradient; //渐变效果
    private Paint mPaint; //画笔
    private ObjectAnimator mRadiusAnimator; //属性动画

    private int dp(int dp) {
        return (int) (dp * mDensity + 0.5f);//dp转px
    }
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN //多点触控
